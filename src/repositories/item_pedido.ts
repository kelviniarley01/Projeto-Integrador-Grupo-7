...
export class ItemPedidoRepository {
    salvar(i: item_pedido): item_pedido {
        const r = db.prepare('INSERT INTO itens_pedido (id_pedido, id_produto, quantidade, preco_unitario) VALUES (?, ?, ?, ?)')
    ...
    listar() {
        return db.prepare('SELECT * FROM itens_pedido').all();
    }

    listarPorPedido(id: number) {
        return db.prepare('SELECT * FROM itens_pedido WHERE id_pedido = ?').all(id);
    }
}
